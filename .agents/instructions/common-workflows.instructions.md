...
- Agents: follow the pre-PR checklist in `.agents/instructions/agents.instructions.md` and use the Todo List Tool for multi-step tasks (plan → mark `in-progress` → complete). Always include the commands you ran in the PR description (for example: `uv run ruff check --fix .`, `uv run pytest`).
