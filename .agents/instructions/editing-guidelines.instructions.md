...
- Use `.markdownlint.jsonc` if present for compatibility, but prefer `rumdl` for Markdown linting and formatting. Add `rumdl` to `[dependency-groups].dev` in `pyproject.toml` and run `uv sync` to install it into the project `.venv`; then use `uv run --locked rumdl check`, `uv run --locked rumdl check --fix`, or `uv run --locked rumdl fmt`.
