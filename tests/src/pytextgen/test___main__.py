"""Tests for :mod:`pytextgen.__main__` entry-point behavior."""
...
async def test_main_parses_argv_and_invokes_entry(
    ...
    monkeypatch.setattr(pkg_main, "parser", lambda: parser)
    monkeypatch.setattr(pkg_main, "argv", ["pytextgen", "generate", "in.md"])

    await pkg_main.main()

    assert parser.args == ["generate", "in.md"]
...
def test___main___script_guard_uses_runnify(run_module_helper: RunModuleHelper) -> None:
    ...
    called = run_module_helper("pytextgen.__main__", ["pytextgen"])
